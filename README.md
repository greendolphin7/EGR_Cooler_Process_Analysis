...
그 제품이 하는 역할과 작동원리, 품질에서 기대하는 기대치와 허용치를 모두 다 알아야 한다.  // 2020.08.19
...
각 중점, 설비, 데이터 수집 장비별로 더 세분화해서 선별할 수 있다. // 2020.08.20
...
부적합품을 만들어내는 Factor 들을 추정하거나 서로 다른 데이터들의 상관관계와 인과관계를 유추해볼 수 있도록 한다. // 2020.08.22
...
각 데이터들에 대한 형태(binary, distribution)와 부적합품 기준(허용공차)에 대한 논의를 거친 후 데이터들을 랜덤으로 생성할 계획이다. // 2020.08.23
...
- 각 공정에서의 제품에 대한 품질 판단을 실시한다고 가정하고, 다음공정으로 넘어가기 전에 양품과 부적합품을 가려낸다. (0, 1)  // 2020.08.24
...
3) 모델의 데이터를 가지고 어떤 결과를 내고 싶은지에 대해 더 고민해보아야겠다. // 2020.08.25
...
내일은 온도에 대한 op 40 ~ op 50 까지의 공정을 통해 최종적인 값을 알아내고, 양품 (0, 1)을 판별할 수 있는 데이터 셋을 완성시켜 보아야겠다. // 2020.08.26
...
앞으로는 XGboost과 같은 다른 알고리즘을 통해 학습시켜 결과값을 비교해보도록 하겠다. // 2020.08.27
...
만약 모든 부품의 치수가 이상적으로 똑같다고 가정하고 공정과정에서 나온 결과값을 구한다면 그 산포가 어느정도인지를 구할 수 있을것이고 그 산포를 비교해보면 될까? // 2020.08.29
...
다음주 월요일부터는 설비의 가동시간, 비가동시간과 관련된 상황들과 그에 맞는 데이터들을 생각하고 추가해보는 시간을 가져야겠다. // 2020.08.30
