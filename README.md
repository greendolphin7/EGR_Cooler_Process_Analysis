...
각 상황과 설비에 맞는 데이터들을 뽑아와서 최종 분석하기 전까지의 데이터들을 올바르게 받아와 정리까지 한 결과로 엑셀이라는 파일로 만들었다는 시나리오로 진행되고 있다.
현실에서는 분석하기 위한 하나의 csv파일을 만들기까지 각각의 설비에 부착된 센서나 하드웨어들에서 데이터들을 추출해서 그것들을 데이터베이스에 저장한 후 변환, 적재 (ETL)과정을 거친 후가공된 형태로 만든 후일 것이다. 
